...
public class AlumnoRepositoryImpl implements AlumnoRepository {
    ...
    private static List<Predicate> crearElFiltro(final CriteriaBuilder criteriaBuilder, final Root<Alumno> tablaPrincipal,
        ...
        if (StringUtils.isNotEmpty(filtro.getCiclo())) {
            final Join<Alumno, Ciclo> alumnoCicloJoin = tablaPrincipal.join("ciclo", JoinType.INNER);
            condiciones.add(criteriaBuilder.equal(alumnoCicloJoin.get("codigo"), filtro.getCiclo()));
        }
        ...
        if (filtro.getCursoMinimo() != null) {
            //predicates.add(criteriaBuilder.greaterThanOrEqualTo(tablaPrincipal.get("curso"), filtro.getCursoMinimo()));
            condiciones.add(criteriaBuilder.ge(tablaPrincipal.get("curso"), filtro.getCursoMinimo()));
        }

        if (filtro.getCursoMaximo() != null) {
            //predicates.add(criteriaBuilder.lessThanOrEqualTo(tablaPrincipal.get("curso"), filtro.getCursoMaximo()));
